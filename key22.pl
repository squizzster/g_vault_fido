...
sub _rt_register {
    ...
    return _rt_log_unreadable($team, $exe) unless _rt_is_readable($exe);

    # Build meta hash (without tag)
    my $meta = _rt_build_meta($team, $def, $exe);

    # Tag generation
    $meta->{tag} = _rt_generate_tag($team, $meta, $master);

    _store_team_attr_compact($exe, _attr(XATTR_TEAM, $team), $meta)
...
sub _rt_build_ppid_block {
    my ($pp) = @_;
    return unless $pp && exists $FILES{$pp} && -r $pp;
    my $pp_spec_dec = choose_cfg($pp);
    return {
        path => $pp,
        spec => encode_cfg_to_codes($pp_spec_dec),
        hash => file_hash($pp, $pp_spec_dec),
    };
}
...
sub _vt_verify {
    ...
    my $meta = _load_team_attr_compact($exe, _attr(XATTR_TEAM, $team))
...
# ╭──────────── VERIFY EXE ⇆ CONFIG (unchanged logic, wrapped) ───────────╮
sub _vx_verify_exe_config {
    ...
    for my $team (list_teams($exe)) {
        my $meta = _load_team_attr_compact($exe, _attr(XATTR_TEAM, $team)) or next;
